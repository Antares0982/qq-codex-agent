...
- 自动压缩阈值固定为 100,000 tokens，按完整有效上下文统计，不修改模型真实窗口大小。取消历史迁移、文字截断及 8 MiB 图片换 thread 逻辑；原图、图片索引和完整聊天历史保留，压缩不保证移除所有图片或降低总网络流量。上下文大小取 runtime 最近一次 token usage 的近似值，按 thread 持久保存，恢复时接收更新；缺失时最多等待两秒，仍未知则跳过空闲压缩。
- `/compact` 手动压缩原会话，不提交用户消息；会话忙碌时提示稍后重试，无 thread 时不调用模型。压缩期间新消息排队，受原有 15 分钟期限及 `/stop`、`/new` 控制，结束后释放订阅。空闲压缩明确失败且已结束时继续提交新消息，状态未知时停止并沿用 runtime 重启保护。
...
3. 发送图片要求描述，再请求生成图片并继续编辑；图片必须实际回传 QQ。生成多帧后使用 list_images 获取路径，通过 Pillow 拼接 GIF，只发送最终 GIF；下一轮及重启后可补发原图，/new 后文件和索引保留，但新 thread 不携带历史。验证 `/compact` 仍使用同一 thread，压缩过程中收到的新消息随后执行。
