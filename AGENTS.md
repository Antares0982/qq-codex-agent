...
- 私聊与各群并发执行，各聊天独立调度。同一聊天处理中收到的新消息按到达顺序通过 steering 追加到当前 turn，由 runtime 在工具调用结束等可接收输入的位置处理；如果 turn 已结束，则开启下一轮。每个聊天最多等待 8 条未提交消息，每条最多 5 张图片、每张 10 MiB，每轮最长 15 分钟，追加消息不延长超时。
